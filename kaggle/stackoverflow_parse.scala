...
object stackoverflow_parse
{
  ...
  def printAsLibSVM (x: RDD [(Double, Double, Double, Double, Double, Int)], filename: String) =
  {
    val s = new PrintStream (new FileOutputStream (filename))
    x.collect.foreach { case (d1, d2, d3, d4, d5, i) => s.println (i + " 1:" + d1 + " 2:" + d2 + " 3:" + d3 + " 4:" + d4 + " 5:" + d5) }
    s.close
  }
